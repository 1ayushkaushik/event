...
const BudgetForm: React.FC<BudgetFormProps> = ({ onSubmit }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm({ ...form, [name]: name === 'guestCount' || name === 'totalBudget' ? Number(value) : value });
  };
  ...
  return (
    ...
    >
      ...
      <div className="pt-3 flex justify-center">
        <button
          type="submit"
          className="bg-violet-600 text-white px-6 py-2 rounded-md text-sm hover:bg-violet-700 transition"
        >
          Continue
        </button>
      </div>
